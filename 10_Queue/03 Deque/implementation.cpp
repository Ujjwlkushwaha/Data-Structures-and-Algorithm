...
{
    // step 1 : queue is full or not 
    if ((f == 0 && r == size - 1) || (f == r + 1))
    {
        printf("Overflow");
    }
    // step 2 : if queue has no element
    else if ((f == -1) && (r == -1))
    {
        f = r = 0;
        deque[f] = x;
    }
    // if front is placeed at first position 
    else if (f == 0)
    {
        f = size - 1;
        deque[f] = x;
...
void insert_rear(int x)
{
    ...
    // cheeck queue is empty
    else if ((f == -1) && (r == -1))
    {
        r = 0;
        deque[r] = x;
    }
    // rare at last position and front is not in first place
    else if (r == size - 1)
    {
        r = 0;
        deque[r] = x;
